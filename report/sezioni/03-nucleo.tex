...
\subsection{Ciclo di vita del kernel}\label{sec:tre-fasi}
...
\begin{itemize}
    \item \code{local\_gemm\_create} ne inizializza il contesto,
    \item \code{local\_gemm} esegue il calcolo e
    \item \code{local\_gemm\_destroy} rilascia le risorse associate.
\end{itemize}
...
\begin{enumerate}
    ...
    Nei backend CUDA vengono allocati i buffer sul device e
