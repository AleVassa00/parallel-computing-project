...
\section{Che cosa significa parallelizzare in MPI}
\label{sec:mpi-intro}
...
\begin{itemize}[nosep,left=0pt]
  \item \textbf{costringerebbe a replicare $A$.} Per calcolare un qualunque
        ...
        identico blocco di $A$. Si duplicherebbe il dato dominante
        (\cref{sec:traffico}), che è l'opposto di ciò per cui la decomposizione
        esiste;
...
stessa con cui se ne tagliano le righe. Non è un controllo da eseguire, è una
proprietà garantita \emph{per costruzione}: esistono soltanto $\mloc$ e
$\nloc$, calcolati una volta sola in \code{layout\_init}, e non due coppie di
valori da tenere allineate. La struttura \code{layout\_t} è il punto unico di
verità sulla forma dei blocchi, e finché resta tale l'invariante non può essere
violato senza modificarla.
\end{tcolorbox}
