...
byte, dove la dimensione in
byte dello scalare $s$ è la dimensione in
byte di ogni elemento della matrice, e $N$ è la dimensione che la traccia chiede di far crescere: il caso
...
riga di $A$ è contigua per costruzione; la riga di $X$ lo è perché $X$ è
...
da $(\text{seme}, \text{chiave})$ al valore in un passo solo.

La chiave scelta è l'\textbf{indice lineare globale} dell'elemento:
\begin{equation}
  ...
  \label{eq:gen}
\end{equation}
dove $\Sigma_A$ e $\Sigma_X$ sono due identificatori di stream distinti, che
servono a evitare che $A$ e $X$ abbiano lo stesso pattern di valori. Il seme
