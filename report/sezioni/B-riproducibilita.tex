...
make KERNEL=cuda_warp X_LAYOUT=column # X compatta per colonne (solo cuda_warp)
make KERNEL=cuda_warp_smem
make KERNEL=cuda_warp_smem SMEM_PAD=0
make KERNEL=cuda_warp_smem TILE_GRANULARITY=8
make KERNEL=cuda_warp_smem SMEM_BUDGET_BYTES=16384
make KERNEL=cuda_warp_tiled
make KERNEL=cuda_warp_tiled WARP_COL_TILE=4
make KERNEL=cublas                    # aggiunge -lcublas da solo
\end{lstlisting}

La riga finale di ogni \code{make} ricorda quale binario ha prodotto e con quali
variabili, budget di shared memory incluso (\code{derivato} quando non \`e
forzato).
...
./script/run_cuda_experiments.sh --suite experiments/c7_1_precision_k.txt
./script/run_cuda_experiments.sh --suite experiments/c7_2_precision_resources.txt
\end{lstlisting}

C7.2 non esegue benchmark: ricompila con \code{-Xptxas -v} e raccoglie i log
delle risorse. I casi $k$ vanno selezionati dal log completo
\file{*\_ptxas\_*.txt}, non dal solo \file{*\_registers.txt}, che omette i nomi
delle funzioni e quindi non permette di attribuire i registri al $k$ giusto.
...
\begin{lstlisting}[language=shell]
cd report && ./compila.sh   # oppure: latexmk -pdf main.tex
\end{lstlisting}
