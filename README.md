...
```bash
mpirun -np 4 ./bin/matmul_mpi -M 8000 -N 8000 -k 8 --pr 2 --pc 2 --reps 10
```
...
fattorizzazione piu' quadrata di P), `--reps --warmup`, `--seed`, `--check`
(validazione contro il seriale), `--csv` / `--csv-header` per la campagna di
misura. `--help` per l'elenco completo.
