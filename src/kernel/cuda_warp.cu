...
local_gemm_t *local_gemm_create(int m_loc, int n_loc, int k, const scalar_t *A_loc, int lda, int ldx, int ldy) {
    ...
    if (ldx > 0 && (size_t)n_loc > SIZE_MAX / sizeof(scalar_t) / (size_t)ldx)
    ...
    local_gemm_context->bytes_h2d_X_per_call =
        (n_loc > 0 && k > 0)
          ? (size_t)n_loc * (size_t)ldx * sizeof(scalar_t) : 0;
...
void local_gemm(local_gemm_t *local_gemm_context, const scalar_t *RESTRICT X_loc, int ldx, scalar_t *RESTRICT Y_loc_part, int ldy) {
    ...
    if (m_loc > 0 && k > 0) {
        launch_warp_kernel(m_loc, n_loc, k, local_gemm_context->dA_loc, local_gemm_context->lda, local_gemm_context->dX_loc, local_gemm_context->ldx, local_gemm_context->dY_loc_part, ldy);
